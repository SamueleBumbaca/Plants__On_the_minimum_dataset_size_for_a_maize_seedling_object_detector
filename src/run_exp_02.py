...
def main(experiment):
    ...
    with open(csv_path, mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            trian_size = float(row['train_size'])
            val_size = float(row['val_size'])
            tsp = int(dataset_size*trian_size)
            vsp = int(dataset_size*val_size)
            yolo_dataset_folder = f'train_{tsp}_val_{vsp}'
