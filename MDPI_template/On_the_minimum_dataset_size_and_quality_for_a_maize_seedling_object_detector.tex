...
to extract the in-domain dataset for training a DL model.
...
Dataset size and quality predictability in DL is a controversial topic that has been proven to be
...
amount of data used for training \cite{sunRevisitingUnreasonableEffectiveness2017}, 
its quality \cite{alhazmiEffectsAnnotationQuality2021}.
The model architecture is critical in dataset requirements as different models may require
...
Backbone is pivotal on downstream tasks as object detection. 
...
object detector is the dataset source. It has been observed that using training samples from the same
...
Plant counting is a challenging task, mostly because of data scarcity: public datasets
are rare and often not suitable for the task because of the large environmental
variability, their images are not orthorectified or scale is unknown. Even so we can find 
some datasets that can be used for training a plant counting object detector.
