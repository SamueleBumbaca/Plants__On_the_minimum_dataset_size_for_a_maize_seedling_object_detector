...
% The template details the sections that can be used in a manuscript. Note that the 
% order and names of article sections may differ from the requirements of the journal 
% (e.g., the positioning of the Materials and Methods section). Please check the instructions 
% on the authors' page of the journal to verify the correct order and names. For any 
% questions, please contact the editorial office of the journal or support@mdpi.com. 
% For LaTeX-related questions please contact latex@mdpi.com.%\endnote{This is an endnote.} % To use endnotes, please un-comment \printendnotes below (before References). Only journal Laws uses \footnote.
...
% The current state of the research field should be reviewed carefully and key publications 
% cited. Please highlight controversial and diverging hypotheses when necessary. Finally, 
% briefly mention the main aim of the work and highlight the principal conclusions. 
% As far as possible, please keep the introduction comprehensible to scientists outside 
% your particular field of research. Citing a journal paper \citep{ref-journal}.  
% Now citing a book reference \citep{ref-book1,ref-book2} or other reference types 
% \citep{ref-unpublish,ref-url}. Please use the command \citep{ref-proceeding,ref-thesis} 
% for the following MDPI journals, which use author--date citation: Administrative Sciences, 
% Arts, Behavioral Sciences, Businesses, Econometrics, Economies, Education Sciences, 
% European Journal of Investigation in Health, Psychology and Education, Games, Genealogy, 
% Histories, Humanities, Humans, IJFS, Journal of Intelligence, Journalism and Media, JRFM, 
% Languages, Laws, Literature, Psychology International, Publications, Religions, Risks, 
% Social Sciences, Tourism and Hospitality, Youth. 
...
or by comparison with other methods.
Accuracy of plant manual counting depends on human performance, so difficult to 
define, but it is often taken as golden sample. 
...
Counting of plants is then commonly performed by counting the number of bounding boxes 
...
that involves capturing overlapping aerial georeferenced images taken by nadiral view
(and/or picturing georeferenced ground control points), and performing boundle adjustment to form 
...
It implies that the pixel coordinates and size correspond to 
...
now a days. Nethertheless, HC methods are still used in some cases \cite{davidPlantDetectionCounting2021}. 
Modern DL object detection uses CNN-based frameworks (e.g., Faster R-CNN \cite{FasterRCNNRealTime}, YOLO \cite{YouOnlyLook}) 
and Transformer-based approaches (e.g., DETR \cite{carionEndtoEndObjectDetection2020}).
...
no labeled examples respectivelly. The term "shot" refers to the number of annotations (plants for this study) 
used to train the model over all the images.
...
Some study focused on few-shots approach for plant counting 
CiteHere(One to All: Toward a Unified Model for Counting Cereal Crop Heads Based on Few-Shot Learning)
CiteHere(AdaTreeFormer: Few shot domain adaptation for tree counting from a single high-resolution image)
but only two specifically accounting on few-shots method for maize seedling counting \cite{karamiAutomaticPlantCounting2020}
CiteHere(Advancing Image Recognition: Towards Lightweight Few-shot Learning Model for Maize Seedling Detection).
These lasts set the few-shot benchmark for maize seedling counting at 600 examples.
No zero-shot benchmark for maize seedling counting has been set yet, and no research has been done yet.
...
as for the most of the specialized domains. So, because of limited resourches, 
ùonly the use of a general backbone is possible for this study, 
even if a decreasing in dataset size requirements is expected with a domain specific 
backbone \cite{roggiolaniDomainSpecificPreTraining2023}.

Nethertheless, the most important factor affecting the dataset size for a plant counting
object detector is the dataset source \cite{davidPlantDetectionCounting2021,andvaagCountingCanolaGeneralizable2024}. 
It has been observed that using training samples from the same
dataset as the inference (in-domain dataset) increases accuracy and reduce the need 
...
Plant counting is affected as many DL application fields from data scarcity: public datasets
are rare and often not suitable for the task because of the large environmental
variability CiteHere , their images are not orthorectified or scale is unknown CiteHere . 
Even so we can find some datasets that can be used for training a plant counting object detector CiteHere .
We can reduce the variability that the model should learn by constrain the dataset
to a specific crop at a specific growth stage. For this study we choose maize seedlings
(Zea mais L.) at the V3-V5 (BBCH 13-15) growth stage. It is a common crop 
that in this particular growth stage is easy to count because of low overlapping
and fixed intra-row and inter-row spacing.

This study aims to determine the minimum dataset size and quality required to train 
a benchmark reaching object detection model for identifying maize seedlings in georeferenced orthomosaics,
...
% Materials and Methods should be described with sufficient details to allow others 
% to replicate and build on published results. Please note that publication of your 
% manuscript implicates that you must make all materials, data, computer code, and 
% protocols associated with the publication available to readers. Please disclose 
% at the submission stage any restrictions on the availability of materials or information. 
% New methods and protocols should be described in detail while well-established methods 
% can be briefly described and appropriately cited.

% Research manuscripts reporting large datasets that are deposited in a publicly avail-able 
...
% of submission, please state that they will be provided during review. They must 
...
where with recursivelly fit a series of models with increasing dataset size and quality.
In this section we will first define the datasets used in the experiments, divided by 
in-domain and out-of-distribution training datasets. 
Then we will describe the architectures chosen to rapresent the 
many-shots, few-shots, and zero-shots approaches.
Finally we will describe a HC object detection algorithm to evaluate 
it as a strategy to get the training datasets.
...
leaf is unfolded and the plant is 15-30 cm tall \cite{meierBBCHMonograph2018}.
This growth stage is easy to determine, even from above because of the low overlapping
of the leafs. 
...
The ID datasets were collected during this study.
...
\midrule
OOD Scientific & Composed of tiles of georeferenced orthomosaics of maize seedlings from scientific literature \cite{liuEstimatingMaizeSeedling2022,davidPlantDetectionCounting2021}. \\
OOD Internet & Composed of RGB images of maize seedlings from internet repositories \cite{Maize_seedingDatasetOverview,MaizeseedlingdetectionDatasetOverview}. \\
...
All the datasets were then cropped to 224*224 pixels tiles.
Also the annotations were rescaled where needed and cropped.  

The testing dataset is composed by a subset of the ID dataset tiles, not included in the training dataset.

\subsection{many-shots object detectors}

To rapresent the category and compare performance between pure-CNN and Transformer-mixed architectures
that are effectivelly used as object detectors in real use-cases for their accuracy and efficiency,
we chose to use three versions of YOLO and the real-time version of DETR in the Ultralytics implementation.
The YOLO versions are YOLOv5, YOLOv8, and YOLO11, while the DETR version is RT-DETR.

YOLOv5 CiteHere is a pure CNN-based architecture that represents a significant optimization 
over previous YOLO versions with improved speed-accuracy tradeoffs and enhanced training stability. 
It employs a CSPDarknet53 backbone that uses cross-stage partial connections to reduce computational 
costs while maintaining feature representation power. YOLOv8 CiteHere introduced 
architectural modifications including a decoupled detection head, a C2f module for feature processing, and several 
advanced augmentation techniques, resulting in better performance on small objects like seedlings. 
Its backbone is an improved CSPDarknet with enhanced SPPF (Spatial Pyramid Pooling - Fast) modules for 
multi-scale feature extraction. YOLO11 CiteHere represents the latest evolution with significant 
improvements in accuracy through adaptable block and module designs, adaptive head assignments, and enhanced 
loss functions for optimizing detection at various scales. YOLO11 uses an advanced adaptable backbone with 
improved feature pyramid networks for efficient multi-scale detection. RT-DETR CiteHere is a real-time 
transformer-based detector that combines CNN backbone features (ResNet-based) with a streamlined 
transformer decoder, striking a balance between the global reasoning capabilities of transformers and the efficiency 
of CNN architectures, making it particularly effective for detecting objects with varying sizes and densities 
in complex agricultural scenes.

We used here the Ultralytics implementation of these models because the implementation is open-source CiteHere
and it is widely used in the scientific community CiteHere.

MakeTableHere

\subsection{few-shots object detectors}

Differently from the many-shots approach, the few-shots one was not deeply studied to tackle the problem of plant counting.
In fact
To test if few-shot approach can effectivelly reduce the amount of training samples to train a plant counting object detector
we decided to use the state-of-the-art disputed cross-domain method few-shot model that have shown promising results in the literature.
CD-ViTO is an advanced few-shot object detection model that specifically addresses the challenges of domain shift 
between base and novel classes. The model employs a dual-branch vision transformer backbone that separately processes 
visual and textual information, with a specialized feature fusion mechanism to integrate these modalities. CD-ViTO's architecture 
contains a domain-adaptive feature alignment module that minimizes the distribution discrepancy between source and target 
domains through adversarial learning techniques. Unlike previous few-shot approaches that rely primarily on meta-learning, 
CD-ViTO leverages cross-domain adaptation strategies to better generalize from base to novel classes with minimal 
examples (typically 1-10 shots). The backbone consists of a modified ViT (Vision Transformer) structure with domain-invariant 
attention layers that help extract features robust to domain variations. In benchmark evaluations, CD-ViTO has demonstrated 
superior performance compared to previous few-shot object detection methods, achieving up to 15\% higher AP scores on 
cross-domain detection tasks and showing particular strength in agricultural applications where domain shifts are 
common due to variations in growth conditions, illumination, and plant morphology.
...
Many HC object detectors for maize seedlings count have been proposed 
in the literature. Unlike other works \cite{davidPlantDetectionCounting2021} 
we wrote an HC algorithm to get accurate annotated dataset, 
sacrifing the amount of images succesfully detected
for the quality of the annotations. 

This HC algorithm is havelly based on agronomical knowledge and color thresholding.
For the execution of this algorithm, the user must set the following parameters:

\begin{itemize}
    \item HSV minimum and maximum thresholds
    \item minimum and maxiumum leaf area for plant
    \item minimum distance between plants on rows
    \item distance inter rows
\end{itemize}
...
% Please punctuate equations as regular text. Theorem-type environments (including propositions, 
...
% Text of the proof. Note that the phrase ``of Theorem 1'' is optional if it is clear 
% which theorem is being referred to.
% \end{proof}
% The text continues here.
...
the research shown; figures of replicates for experiments of which representative 
data are shown in the main text can be added here if brief, or as Supplementary Data. 
