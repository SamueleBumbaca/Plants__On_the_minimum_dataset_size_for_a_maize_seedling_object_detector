...
\midrule
OOD Scientific && \\
- DavidEtAl.2021 \cite{davidPlantDetectionCounting2021} & V3 & 182 tiles & None & [0,0,0] & [0,0,0] \\
- LiuEtAl.2022 \cite{liuEstimatingMaizeSeedling2022} & V3 & 596 tiles & None & [0,0,0] & [0,0,0]  \\
OOD Internet && \\
- OOD_int_1 \cite{Maize_seedingDatasetOverview} & V3 & 216 tiles & None & [0,0,0] & [0,0,0]  \\
- OOD_int_2 \cite{MaizeseedlingdetectionDatasetOverview} & V5 & 174 tiles & None & [0,0,0] & [0,0,0]  \\
ID && \\
- ID\_1 & V3 & 150 tiles & 20 tiles & [0,0,0] & [0,0,0]  \\
- ID\_2 & V3 & 150 tiles & 20 tiles & [0,0,0] & [0,0,0]  \\
- ID\_3 & V5 & 150 tiles & 20 tiles & [0,0,0] & [0,0,0]  \\
\bottomrule
\end{tabularx}
\end{table}
...
For all the OWLv2 variants, we tested multiple text prompts to describe maize seedlings, the complete list of prompts is
in appendix. All the combinations (here named model settings) of encoder size, pre-training strategy, and text prompt 
were tested on the ID test dataset tiles.
...
The highest R-squared value whithin the thresholds was considered as the model 
...
Figure \ref{fig:zeroshots_vs_performance} shows the relationship between the 
zero-shots model settings and model performance tested on ID testing datasets.
Not all the model settings were able to predict the whole testing dataset.
For example the owlv2-base-patch16-finetuned model was not able to generate any
prediction with any prompt for any image of the ID testing. 
A real dataset size relation with metrics could not be established as zero-shots models do not require fine-tuning 
training data. 
...
The RMSE and MAPE were higher than the observed in the many-shots and few-shots models. 
Also the mAP values were lowers than the ones of the many and few-shots models for all 
the model settings, unless for the owlv2-large-patch14-finetuned, for which
very few images were succesfully predicted with a mAP comparable with the one of
the 50-shots ViT-B backbone model.
...
for successful maize seedling detection. None of the tested models—regardless 
of architecture or parameter count—achieved the benchmark R-squared value of 0.85 
when trained on out-of-distribution (OOD) datasets. This aligns with previous findings 
...
This domain gap challenge is particularly pronounced in agricultural applications, 
...
significant adaptation. Higher performances of OOD internet datasets training in
respect to the scientific datasets training could be due to the fact that the internet
datasets are more similar to the ID datasets than the scientific datasets.
In specific the dataset RGB means and standard deviations were closer to the ID\_2 and 
the ID\_3 datasets for the internet datasets than for the scientific datasets.
HERE TALK ABOUT THE CURVE DATASET-SIZE/R2
This even highlights the importance of collecting in-domain training data, 
as the minimum OOD dataset size and quality to train a object detector to
count arable crops seedling that generalize to all the real-world cases
...
Interestingly, increasing model complexity in CNN-based architectures corresponded to increased 
dataset size requirements, which aligns with the general understanding that more complex models 
with higher parameter counts typically require more training data to avoid overfitting.
...
empirical models of dataset size versus performance shown equiparable GoF RT-DETR and YOLO-family models,
unless for YOLOv5n that shown a particularly high GoF. This suggests that transformer-based models have
the same probability to reach the benchmark with the reported dataset size as the CNN-based models, unless
for YOLOv5n that have a higher probability to reach the benchmark with the same dataset size.
...
for successful models) provides practical guidance for practitioners. The logarithmic relationship between 
dataset size and performance suggests diminishing returns beyond certain thresholds, which can help inform 
efficient resource allocation for annotation efforts.
...
in annotation quality while still maintaining benchmark performance. YOLOv5n, YOLOv5s, and YOLOv8n achieved 
...
suggesting different sensitivity to annotation errors across architectures.
...
in general object detection benchmarks [Xu et al., 2023; Minderer et al., 2023]. Several factors 
may explain this gap:

Agricultural imagery presents unique challenges including high intra-class variation (plants at 
slightly different growth stages), low inter-class distinction (similar green vegetation), and 
domain-specific features not well-represented in general pre-training datasets.

The precision requirements for plant counting (R-squared ≥ 0.85) represent a significantly higher 
bar than typical object detection tasks, where approximate localization may be sufficient.

The geometric arrangement of plants in rows creates contextual patterns that may not be well-captured 
by models primarily designed for general object detection.

While the predictable relationship between shot count and performance (with high GoF values) suggests 
potential for improvement with more shots, extrapolation indicates that unreasonable numbers of shots 
would be needed to reach benchmark performance with current few-shot architectures.
...
demonstrated strong performance on the testing datasets (R-squared from 0.87 to 0.95). 
...
For achieving benchmark performance, collecting in-domain training data is non-negotiable; 
practitioners should prioritize obtaining representative samples from their specific deployment 
environments.
...
after which additional annotations yield diminishing returns.

Lightweight models like YOLOv8n provide a good balance between performance and data efficiency, making 
them suitable for agricultural applications with limited annotation resources.

Current few-shot and zero-shot methods are not yet viable replacements for traditional object detection 
approaches in precision agriculture applications requiring high accuracy.

\subsection{Future Research Directions}
Several promising directions emerge from this work:

Domain adaptation techniques to bridge the gap between OOD and ID datasets, 
potentially reducing the need for extensive in-domain annotation.

Development of agriculture-specific pre-training approaches and backbones 
that better capture the unique features of plant imagery.

Few-shot architectures specifically designed for precision agriculture, 
incorporating domain knowledge about plant morphology and field layout.

Hybrid approaches combining the precision of handcrafted methods with the 
generalizability of deep learning models.

Investigation of annotation quality-quantity trade-offs, potentially developing 
automated quality assessment tools to optimize annotation workflows.

Exploration of temporal consistency and multi-modal data to improve detection 
robustness across growth stages.

Future work should also extend these investigations to other crop types and growth 
stages to develop more generalizable principles for minimum dataset requirements in agricultural object detection.
...
training data, with out-of-distribution datasets failing to achieve benchmark performance 
across all tested models. We established minimum dataset requirements for several 
...
meet agricultural precision requirements for maize seedling detection. Our handcrafted 
...
findings provide practical guidance for developing maize seedling detection systems 
in precision agriculture, emphasizing the importance of in-domain data collection, 
appropriate architecture selection based on available resources, and strategic annotation 
efforts focused on achieving minimum viable dataset size and quality.
...
% The slope and the intercept of the second line are then compared with the first line
% to check if they are parallel and at the right distance. If the conditions are met
% the centers of the second line are considered as the centers of the plants. The
% algorithm is then able to count the number of plants in the tile.
